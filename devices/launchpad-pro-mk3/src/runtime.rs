...
use crate::map::{BUTTON_REMAP, BUTTON_REMAP_SIZE};
...
impl RuntimeDriver {
    pub fn new(
        ...
        Self {
            link,
            leds: crate::led::LedSystem::new_legacy(),
            flash,
            brightness: 7,
    ...
    fn apply_m0_firmware_status(&mut self, status: &M0FirmwareStatus) {
        ...
        match status.kind {
            M0_FW_ROADRUNNER => {
                ...
                self.leds = crate::led::LedSystem::new_highspeed();
            }
            M0_FW_LEGACY => {
                self.m0_firmware_kind = M0_FW_LEGACY;
                self.leds = crate::led::LedSystem::new_legacy();
            }
            _ => {
...
impl Driver for RuntimeDriver {
    ...
    fn fill(&mut self, color: u32) {
        for led in 0..crate::map::LED_REMAP_SIZE as u8 {
    ...
    fn send_midi(&mut self, port: firmware_core::sys::midi::MidiPort, data: &[u8]) {
