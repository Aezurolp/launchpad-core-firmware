...
async fn usb_midi_task(driver: Driver<'static, peripherals::USB_OTG_FS>) {
    let mut config = Config::new(0x1235, 0x0113);

    config.device_release = 0x0200;
    config.manufacturer = Some("Focusrite - Novation");
    config.product = Some("Launchpad Mini MK3");
    config.serial_number = Some("COREFW-MINIMK3");
...
fn encode_usb_midi_packets(port: u8, data: &[u8], out: &mut [u8; 64]) -> usize {
    if data.is_empty() {
        return 0;
    }
...
fn encode_sysex_packets(cable: u8, data: &[u8], out: &mut [u8; 64]) -> usize {
    let mut src = 0usize;
    let mut dst = 0usize;
